...
def performance_indicator(preprocessed_filenames_dict, _, output_dir, start_time):
    ...
    roughness_of_actuation = float(np.max(np.abs(f)))

    # Write result yaml file
    filepath = path.join(output_dir, 'roughness_of_actuation.yaml')
    with open(filepath, 'w+') as result_file:
        yaml.dump({
            'type': 'scalar',
            'value': roughness_of_actuation,
...
if __name__ == '__main__':
    arg_len = 3
    script_name = 'roughness_of_actuation'
    if len(argv) != arg_len:
